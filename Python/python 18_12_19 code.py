...
# out_img: (?, 14, 14, 32)
W1 = tf.Variable(tf.random_normal([3, 3, 1, 32], stddev = 0.001), name = 'weight1')
...
W2 = tf.Variable(tf.random_normal([3, 3, 32, 64], stddev = 0.001), name = 'weight2')
...
W3 = tf.Variable(tf.random_normal([3, 3, 64, 128], stddev = 0.001), name = 'weight3')
...
a = sess.run(accuracy, feed_dict = {X : mnist.test.images, Y : mnist.test.labels, keep_prob : 1.0})
print('accuracy :', a)

# 랜덤 1개 값 예측
r = random.randint(0, mnist.test.num_examples - 1)
print('random :', r, 'Label :', sess.run(tf.argmax(mnist.test.labels[r:r+1],1)))
print('Prediction :', sess.run(predict, feed_dict = {X : mnist.test.images[r:r+1], keep_prob : 1.0}))
